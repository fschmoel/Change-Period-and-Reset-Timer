...
void prvSetupTimerInterrupt( void )
{
    ...
    // Configure Watchdog Timer for Interrupt Mode
    cli(); // Disable global interrupts

    // Set Watchdog Change Enable (WDE) and Watchdog Interrupt Enable (WDIE)
    WDTCSR = (1 << WDCE) | (1 << WDE); // Enable configuration mode
    WDTCSR = (1 << WDIE) | portUSE_WDTO; // Set interrupt mode and timeout

    sei(); // Re-enable global interrupts
}

//wdt_enable(portUSE_WDTO );
